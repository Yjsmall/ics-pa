...
void menu_add_option_modules(void) {
  if (modules_sym)
    zconf_error("symbol '%s' redefines option 'modules' already defined by symbol '%s'", current_entry->sym->name, modules_sym->name);
    ...
    switch (prop->type) {
      case P_DEFAULT:
        if ((sym->type == S_STRING || sym->type == S_INT || sym->type == S_HEX) &&
          ...
          prop_warn(prop, "default for config symbol '%s'"
      ...
      case P_IMPLY:
        ...
        if (sym->type != S_BOOLEAN && sym->type != S_TRISTATE)
          prop_warn(prop, "config symbol '%s' uses %s, but is "
                          "not bool or tristate",
                    sym->name, use);
        else if (sym2->type != S_UNKNOWN && sym2->type != S_BOOLEAN && sym2->type != S_TRISTATE)
          prop_warn(prop, "'%s' has wrong type. '%s' only "
...
static void get_prompt_str(struct gstr *r, struct property *prop, struct list_head *head) {
  ...
  if (head && location) {
    ...
    if (list_empty(head))
      ...
      jump->index = list_entry(head->prev, struct jump_key, entries)->index + 1;
  ...
  if (i > 0) {
    ...
    for (j = 4; --i >= 0; j += 2) {
      ...
      if (menu->sym) {
        str_printf(r, " (%s [=%s])", menu->sym->name ? menu->sym->name : "<choice>", sym_get_string_value(menu->sym));
      }
      str_append(r, "\n");
    }
  }
...
static void get_symbol_props_str(struct gstr *r, struct symbol *sym, enum prop_type tok, const char *prefix) {
  bool             hit = false;
  struct property *prop;
